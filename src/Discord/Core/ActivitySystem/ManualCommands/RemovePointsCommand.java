...
public class RemovePointsCommand extends DiscordChatCommand
{
	public static String requiredRole;
	
	public RemovePointsCommand()
	{
		requiredRole = ConfigFile.getPermission("remove_points_command", "daddy god");
	}
	...
	{
		return "removePoints <user> <amount> <reason>";
	}
	
	@Override
	public String getDescription(DiscordCommand sourceCommand, IMessage callerMessage)
	{
		return "Removes points from the specific user (or users) for the weekly score system, the reason is used for logging points earned and lost";
	}
	...
	public void commandExecuted( IMessage message, String[] args )
	{
		if(message.getMentions().size() == 0){
			ChatUtils.sendMessage(message.getChannel(), message.getAuthor().mention() + " Removepoints requires a user input!");
			return;
		}
		...
		for(IUser user : message.getMentions()){
			if(StaffActivity.object.validUser(user)){
				StaffActivity.object.removePoints(user,num,reason);
				num1 += 1;
				
			} else if(UserActivity.object.validUser(user)){
				UserActivity.object.removePoints(user, num, reason);
				num1 += 1;
			}
		}
		...
		if(num1 > 0) {
			ChatUtils.sendMessage(message.getChannel(), message.getAuthor().mention() + " Points have now been removed!");
		}else{
